...
import { Button, Table, Tag, Modal, Form, Input, Select, Popconfirm, Badge, Space, Tooltip, notification, Tabs } from 'antd'
import { PlusOutlined, EditOutlined, DeleteOutlined, ProjectOutlined, LinkOutlined, GithubOutlined, SearchOutlined, ReloadOutlined, PictureOutlined } from '@ant-design/icons'
import { motion } from 'framer-motion'
import { useApi, useMutation } from '../../hooks/useApi'
import { ProjectService } from '../../services/index'
import ProjectImageManager from '../../components/ProjectImageManager/ProjectImageManager'
...
export default function GestionProjets() {
  ...
  const { data: categories, loading, execute: fetchProjects } = useApi(ProjectService.list, { initialData: [] })
  ...
  const createCategoryMutation = useMutation(ProjectService.createCategory, {
    ...
    onSuccess: () => { fetchProjects(); setCatModal(false) }
  })
  const createMutation = useMutation(ProjectService.create, {
    successMessage: '✅ Projet créé !',
    onSuccess: () => { fetchProjects(); closeModal() }
  })
  const updateMutation = useMutation(
    (p) => ProjectService.update(editing?.id, p),
    { successMessage: '✅ Mis à jour.', onSuccess: () => { fetchProjects(); closeModal() } }
  )
  const deleteMutation = useMutation(ProjectService.remove, { successMessage: '🗑 Supprimé.', onSuccess: fetchProjects })

  const closeModal = () => { setModal(false); setEditing(null); setProjectImages([]); setActiveTab('infos') }

  const openCreate = () => { setEditing(null); setProjectImages([]); setActiveTab('infos'); form.resetFields(); setModal(true) }

  const openEdit = (p) => {
    setEditing(p)
    setProjectImages(p.images ?? [])
    setActiveTab('infos')
    form.setFieldsValue({ ...p, project_category_id: p.category?.id, tags: (p.tags ?? []).join(', ') })
    setModal(true)
  }
  ...
  const totalViews = projects?.reduce((a, p) => a + (p.views_count ?? 0), 0) ?? 0
  ...
  const columns = [
    {
      ...
      render: (_, r) => (
        <div className={ls.projCell}>
          {r.images?.length > 0 ? (
            <div className={ls.projThumbImg}>
              <img
                src={r.images.find(i => i.is_cover)?.url ?? r.images[0].url}
                alt={r.title}
              />
              <span className={ls.photoCount}>
                <PictureOutlined style={{ fontSize: 9 }} /> {r.images.length}
              </span>
            </div>
          ) : (
    ...
    {
      title: 'Technologies', key: 'tags',
      render: (_, r) => <Space wrap size={4}>{(r.tags ?? []).slice(0, 3).map(t => <Tag key={t} className={ls.techTag}>{t}</Tag>)}{(r.tags ?? []).length > 3 && <Tag>+{r.tags.length - 3}</Tag>}</Space>
    },
    {
      title: 'Statut', key: 'status',
      render: (_, r) => <Badge status={r.status === 'published' ? 'success' : 'default'} text={<span className={ls.statusText}>{r.status === 'published' ? 'Publié' : 'Brouillon'}</span>} />
    },
    { title: 'Vues', dataIndex: 'views_count', key: 'views', render: v => <span className={ls.views}>{v ?? 0}</span> },
    {
      ...
      render: (_, r) => (
        <Space>
          ...
          <Popconfirm title="Supprimer ?" okText="Oui" cancelText="Non" okButtonProps={{ danger: true }} onConfirm={() => deleteMutation.mutate(r.id)}>
        ...
        <Form form={form} layout="vertical" requiredMark={true} style={{ marginTop: 16 }}>
          <div className={s.formGrid}>
            ...
            <Form.Item name="project_category_id" label="Catégorie" rules={[{ required: true }]} className={s.formGridFull}>
              <Select>{categories.map(cat => <Option key={cat.id} value={cat.id}>{cat.label}</Option>)}</Select>
            </Form.Item>
            <Form.Item name="tags" label="Technologies (virgule)" className={s.formGridFull}><Input placeholder="React, Node.js" /></Form.Item>
            <Form.Item name="short_description" label="Courte description" rules={[{ required: true, message: 'Veuillez ajouter une courte description' }]} className={s.formGridFull}>
      ...
      label: (
        <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          {projectImages.length > 0 && (
            <Badge count={projectImages.length} size="small"
              style={{ background: '#4f8eff', fontSize: 10, marginLeft: 2 }} />
          )}
        </span>
      ),
      children: editing ? (
        <div style={{ paddingTop: 12 }}>
          <ProjectImageManager
            ...
            onChange={(imgs) => { setProjectImages(imgs); fetchProjects() }}
          />
        </div>
      ) : (
        <div className={ls.imagesTabHint}>
          <PictureOutlined style={{ fontSize: 32, opacity: 0.3 }} />
          <p>Créez d'abord le projet, puis revenez dans cet onglet pour ajouter des photos.</p>
          <span>Les photos s'afficheront à la place de l'emoji dans les cartes projets.</span>
        </div>
      )
    }
  ]
    ...
    <div className={s.page}>
      ...
      <div className={s.inner}>
        ...
        <motion.div className={ls.statsRow} {...fadeUp(0.07)}>
          {[
            { l: 'Total',       v: projects.length,                                         c: '#4f8eff' },
            { l: 'Publiés',     v: published,                                                c: '#4ade80' },
            { l: 'Brouillons',  v: projects.filter(p => p.status === 'draft').length,        c: '#f59e0b' },
            { l: 'Avec photos', v: projects.filter(p => (p.images?.length ?? 0) > 0).length, c: '#8b5cf6' },
          ].map(st => (
            <div key={st.l} className={ls.statCard}>
              <span className={ls.statVal} style={{ color: st.c }}>{st.v}</span>
              <span className={ls.statLbl}>{st.l}</span>
            </div>
          ))}
        </motion.div>
      ...
      {/* Modal projet */}
      <Modal open={modal} onCancel={closeModal}
        footer={activeTab === 'infos' ? undefined : null}
        onOk={handleSave}
        confirmLoading={createMutation.loading || updateMutation.loading}
        title={<span style={{ fontFamily: 'var(--font-display)', fontWeight: 700 }}>{editing ? '✏️ Modifier le projet' : '🚀 Nouveau projet'}</span>}
        okText={editing ? 'Enregistrer' : 'Créer'} cancelText="Annuler"
        width={580} centered destroyOnClose>
      ...
      <Modal open={catModal} onCancel={() => setCatModal(false)} onOk={handleCategoryCreate}
        ...
        <Form form={catForm} layout="vertical" requiredMark={true}>
          ...
          <Form.Item name="icon" label="Icône de la catégorie">
            <Input placeholder="⚙️..." />
          </Form.Item>
        </Form>
      </Modal>
